...
const getNextLotteryDrawTime = (currentMillis) => {
  ...
  const nextLotteryIsTomorrow = nextLotteryHour === 0 
...
export const getLotteryDrawTimeMillis = (currentMillis): number => {
  const nextLotteryDrawTime = getNextLotteryDrawTime(currentMillis)
  return nextLotteryDrawTime - currentMillis
}
...
export const getTicketSaleStep = () => 100
