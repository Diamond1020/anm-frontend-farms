...
const PrizeGrid: React.FC<PrizeGridProps> = ({
  ...
  const rewardAmount = +((lotteryPrizeAmount / 100) * 50).toFixed(3)
  // const threeMatchesAmount = +((lotteryPrizeAmount / 100) * 20).toFixed(0)
  // const twoMatchesAmount = +((lotteryPrizeAmount / 100) * 10).toFixed(0)
  // const burnAmount = +((lotteryPrizeAmount / 100) * 10).toFixed(3)
  const ANMPAmount = +((lotteryPrizeAmount / 100) * 10).toFixed(3)
  const nextAmount = +((lotteryPrizeAmount / 100) * 30).toFixed(3)
  ...
  return (
    <Grid pastDraw={pastDraw}>
      ...
      <GridItem>
        <RightAlignedHeading size="md">{rewardAmount.toLocaleString()}</RightAlignedHeading>
      </GridItem>
      {/* 3 matches row */}
      {/* <GridItem>
        <Text bold>3</Text>
      ...
      <GridItem marginBottom="0">
        <Text>{TranslateString(999, `${pastDraw ? 'Buy ANMP and burned' : 'Buy ANMP and burn'}`)}:</Text>
      </GridItem>
      <GridItem marginBottom="0">
        <RightAlignedText>{ANMPAmount.toLocaleString()}</RightAlignedText>
      </GridItem>
      <GridItem marginBottom="0">
        <Text>{TranslateString(999, 'Transfer next round')}:</Text>
      </GridItem>
      <GridItem marginBottom="0">
        <RightAlignedText>{nextAmount.toLocaleString()}</RightAlignedText>
      </GridItem>
    </Grid>
  )
}
