...
export const getPrevTickets = async (lotteryContract, ticketsContract, account, customLotteryNum) => {
  const issueIdex = Number(customLotteryNum || (await lotteryContract.methods.issueIndex().call())) - 1;
  const length = await getTicketsAmount(ticketsContract, account)
  ...
  const finalTokenids = []
  ticketIssues.forEach(async (ticketIssue, i) => {
    if (Number(ticketIssue) === issueIdex) {
      finalTokenids.push(tokenIds[i])
    }
  })
  const calls3 = finalTokenids.map((id) => [ticketsContract.options.address, 'getLotteryNumbers', [id]])
...
export const multiClaim = async (lotteryContract, ticketsContract, account) => {
  ...
  const calls11 = tokenIds.map((id) => [ticketsContract.options.address, 'getLotteryIssueIndex', [id]])
  const ticketIssues = await multiCall(ticketAbi, calls11)
  ...
  ticketIssues.forEach(async (ticketIssue, i) => {
    ...
    if (Number(ticketIssue) < Number(issueIdex) && !claimedStatus[i][0]) {
      unClaimedIds.push(tokenIds[i])
    }
  })
...
export const getWinningNumbers = async (lotteryContract) => {
  ...
  if (!drawed) {
      numbers = (await lotteryContract.methods.historyNumbers(issueIdex - 1).call()).toString()
  } else {
      numbers = (await lotteryContract.methods.winningNumbers().call()).toString()
  }
  return numbers
}
